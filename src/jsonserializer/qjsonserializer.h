...
public:
    ...
    //! Serialize Q_GADGET o QJsonObject
    template <typename T>
    QJsonValue serializeGadget(const T* gadget);

    //! Deserialize Q_GADGET from QJsonValue
    template <typename T>
    QSharedPointer<T> deserializeGadgetSharedPointer(const QJsonValue& jsonValue);
...
template<typename T>
QJsonValue QJsonSerializer::serializeGadget(const T* gadget)
{
    if (!gadget){
        return QJsonObject();
    }
    ...
    for (auto i = 0; i < gadget->staticMetaObject.propertyCount(); i++) {
        auto property = gadget->staticMetaObject.property(i);
        if (property.isStored()){
            jsonObject[QString::fromUtf8(property.name())] = serialize(property.readOnGadget(gadget));
        }
    }
...
template<typename T>
QSharedPointer<T> QJsonSerializer::deserializeGadgetSharedPointer(const QJsonValue &jsonValue)
{
    if (jsonValue.isNull()){
        return nullptr;
    }

    int metaTypeId = qMetaTypeId<T>();

    auto variant = deserialize(jsonValue, metaTypeId);

    return QSharedPointer<T>(new T(variant.value<T>()));
}
